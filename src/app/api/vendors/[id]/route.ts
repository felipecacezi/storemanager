...
export async function GET(
    ...
    try {
        const externalResponse = await fetch(`${auth.apiBaseUrl}/vendors/${id}`, {
            method: "GET",
            headers: {
        ...
        if (!externalResponse.ok) {
            return NextResponse.json(
                { error: payload?.message || "Erro ao buscar fornecedor." },
...
export async function PUT(
    ...
    try {
        ...
        if (!externalResponse.ok) {
            return NextResponse.json(
                { error: payload?.message || "Erro ao atualizar fornecedor." },
                { status: externalResponse.status }
            );
        }
...
export async function DELETE(
    ...
    try {
        ...
        if (!externalResponse.ok) {
            return NextResponse.json(
                { error: payload?.message || "Erro ao remover fornecedor." },
                { status: externalResponse.status }
            );
        }
